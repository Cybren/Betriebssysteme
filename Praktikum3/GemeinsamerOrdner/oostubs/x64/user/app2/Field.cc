...
bool Field::prepareMove(Block* block, int newXPos, int newYPos)
{
    ...
    if (stack.get(0)->id != -1 && stack.get(stack.getObjectCount() - 1)->type == Block::Type::PYRAMID) {
        kout << "Goal position is a pyramid block!" << endl;
        return false;
    }
    Array<Block*> stack2 = this->field[block->posX][block->posY];
    if (stack2.get(0)->id != -1 && stack2.get(stack2.getObjectCount() - 1)->id != block->id) {
