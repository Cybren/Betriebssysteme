...
{
    /**
     * Innerhalb dieser Methode sollen die Befehle, die von der KeybordApllication entgegen genommen wurden,
     * umgesetzt werden.
    ...
    if(action == 1){
        if(id == 1){
            result = field->addCubeBlock(posX, posY);
            if(result == -1){
                return -1;
            }
        }else{
            result = field->addPyramidBlock(posX, posY);
            if(result == -1){
                return -1;
            }
        }
    }else{
        if(!field->moveBlock(id, posX, posY)){
            return -1;
        } else{
            result = id;
        }
